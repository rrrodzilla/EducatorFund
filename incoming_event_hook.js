...
exports.handler = function (context, event, callback) {
    ...
    var amount = 0.0;
    var charge_id = event.data.object.charge;
    var class_name = "";
    ...
    fetchBySharedCustomerPromise.then((data) => {
        ...
        fetchEducatorAccountPromise.then((query_results) => {
            ...
            let educator_stripe_account = educator.stripe_account;

            //now let's send a receipt before sending a notification to the educator
            stripe
                .customers
                .retrieve(customer, {stripe_account: educator_stripe_account})
                .then((cust) => {
                    console.log(cust);
                    console.log("cust.email: " + cust.email);
                    console.log("charge_id: " + charge_id);
                    console.log("educator_stripe_account: " + educator_stripe_account);
                    stripe
                        .charges
                        .update(charge_id, {
                            receipt_email: cust.email
                        }, {stripe_account: educator_stripe_account})
                        .then((charge_updated) => {
                            console.log("charge_updated: " + charge_updated);
                            client
                                .studio
